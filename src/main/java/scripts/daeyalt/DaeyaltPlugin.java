...
public class DaeyaltPlugin extends Script implements Painting, Starting {
    ...
    public void onStart() {
        ...
        runActivation = abcUtil.generateRunActivation();
    }

    @Override
    public void run() {
        while (true) {
            // Run if available. Not too important as even without any stamina related gear you stay at full run energy.
            tryEnsureRunning();
    ...
    /**
     * If the player is not already running, try to enable it based on the suggested activation percentage generated by
     * ABC2.
     */
    private void tryEnsureRunning() {
        if (Game.isRunOn()) {
            return;
        }

        if (Game.getRunEnergy() >= runActivation) {
